...
Driver.prototype.poolQuery = function (query, cb) {
    mysqlH.getConnection(this.config, function(coreObj) {
        coreObj.connection.release();
        coreObj.connection.query(query, function (err, data) {
            return cb(err, data);
        });
    });
};
